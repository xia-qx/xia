package org.xia.common.redis.utils;

import lombok.AccessLevel;
import lombok.NoArgsConstructor;
import org.xia.common.core.utils.SpringUtils;
...
public class QueueUtils {
     ...
     * @param time      延迟时间
     */
    public static <T> void addDxiaedQueueObject(String queueName, T data, long time) {
        addDxiaedQueueObject(queueName, data, time, TimeUnit.MILLISECONDS);
    }
     ...
     * @param timeUnit  单位
     */
    public static <T> void addDxiaedQueueObject(String queueName, T data, long time, TimeUnit timeUnit) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDxiaedQueue<T> dxiaedQueue = CLIENT.getDxiaedQueue(queue);
        dxiaedQueue.offer(data, time, timeUnit);
    }
     ...
     * @param queueName 队列名
     */
    public static <T> T getDxiaedQueueObject(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDxiaedQueue<T> dxiaedQueue = CLIENT.getDxiaedQueue(queue);
        return dxiaedQueue.poll();
    }

    /**
     * 删除延迟队列数据
     */
    public static <T> boolean removeDxiaedQueueObject(String queueName, T data) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDxiaedQueue<T> dxiaedQueue = CLIENT.getDxiaedQueue(queue);
        return dxiaedQueue.remove(data);
    }

    /**
     * 销毁延迟队列 所有阻塞监听 报错
     */
    public static <T> void destroyDxiaedQueue(String queueName) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        RDxiaedQueue<T> dxiaedQueue = CLIENT.getDxiaedQueue(queue);
        dxiaedQueue.destroy();
    }
     ...
     */
    public static <T> void subscribeBlockingQueue(String queueName, Function<T, CompletionStage<Void>> consumer, boolean isDxiaed) {
        RBlockingQueue<T> queue = CLIENT.getBlockingQueue(queueName);
        if (isDxiaed) {
            // 订阅延迟队列
            CLIENT.getDxiaedQueue(queue);
        }
